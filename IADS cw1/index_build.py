...
def mergeFiles(a, b, c):
    xa = ('temp_' + str(a) + '_' + str(b))
    xb = ('temp_' + str(b) + '_' + str(c))
    xc = open('temp_' + str(a) + '_' + str(c), 'w', encoding='utf-8')
    reader_xa = BufferedInput(xa, 0.3)
    reader_xb = BufferedInput(xb, 0.3)

    chunk = reader_xa.readchunk()
    chunk1 = reader_xb.readchunk()
    chunk += chunk1
    chunk.sort()
...
def mergeFilesInRange(a, c):
    ...
    if c-a == 1:
        ...
        mergeFilesInRange(a,x)
        mergeFilesInRange(x,c)
        mergeFiles(a,x,c)
